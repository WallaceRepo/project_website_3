...
//$('.boxdiv').on('click', function(e){});
$('.boxdiv').on('show.bs.dropdown', function() {
		$(this).find('.dropdown-menu').fadeIn();
	
		$('#menu-wrapper #hamburger-menu').addClass('open');
  }); 

/***MENU To switch pages and show expanding preview */
$(function(){
	   
			 $('ul.wwd > li').on('click', function(e){savePage()});
			 $('ul.submenu-list > li ').on('click', function(e){alert("yess")});
			 $('.will > li:not(:first-child)').on('click', function(e){alert("3rddd!")});
	 
			function savePage(){
			 var $clkItem = $(this);
			 var pagehref = $clkItem.parent().data('h');
			 var clkIndex = $clkItem.index();
			 var listInd =  $clkItem.parent().data('listInd');
